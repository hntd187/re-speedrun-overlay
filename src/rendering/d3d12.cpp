...
bool D3D12::initialize() {
...

    {
        ...
        if (FAILED(device->CreateCommittedResource(
                &props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, &clear_value, IID_PPV_ARGS(&this->get_rt(D3D12::RTV::IMGUI))))) {
            return false;
        }

        if (FAILED(device->CreateCommittedResource(
                &props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, &clear_value, IID_PPV_ARGS(&this->get_rt(D3D12::RTV::BLANK))))) {
            return false;
        }
        ...
        device->CreateShaderResourceView(this->get_rt(D3D12::RTV::IMGUI).Get(), nullptr, this->get_cpu_srv(device, D3D12::SRV::IMGUI));
    ...
    if (!ImGui_ImplDX12_Init(device, 1, DXGI_FORMAT_R8G8B8A8_UNORM, this->srv_desc_heap.Get(), this->get_cpu_srv(device, D3D12::SRV::IMGUI_FONT),
            this->get_gpu_srv(device, D3D12::SRV::IMGUI_FONT))) {
        return false;
    }
...
void D3D12::render_imgui() {
    ...
    this->cmd_list->OMSetRenderTargets(1, rts, FALSE, nullptr);
